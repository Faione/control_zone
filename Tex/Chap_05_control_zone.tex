...
时序数据如图~\ref{fig:cpu_series_memcached}所示，包含虚拟机总体的CPU使用率、混部应用各自的CPU使用率以及Memcached的99分位尾延迟，其中红色虚线部分表示stress\_ng CPU负载启动的时间点。在干扰负载启动时段之前，Control Zone中Memcached性能并无明显差异，而在干扰负载启动之后，Control Zone在Memcached延时保障上效果十分明显，实现高优先应用99分尾延迟降低最高63.8\%的效果。
...
而从CPU资源分配的角度上来看，Memcached在Control Zone中运行时能被更优先地分配CPU资源，从而能够更好地进行请求处理，实现延时的降低。与此同时，虚拟机总体CPU使用率确没有明显下降，这得益于Control Tower调度策略对于高优先应用CPU资源使用的感知，通过更合理的CPU分配来保障高优先应用的性能。
