...
相较于Linux任务调度机制，Control Tower任务调度框架能够提供更灵活的任务调度机制与调控手段，并通过定制和组合不同的外部数据来源、调度模式与调度策略来满足不同混部场景应用对调度机制的需求，以更好地保障高优先应用的QoS。
...
实验环境在两台服务上展开，服务器硬件信息如表~\ref{tab:exp_env}所示。实验中考虑到内核兼容性与适配问题，使用Qemu6.2.0与KVM虚拟机作为混部应用的主要运行环境。
...
\begin{table}[H]
    \bicaption{\quad 实验应用及负载}{\quad Applications and Workloads}% caption
    \label{tab:app_work}
    \footnotesize% fontsize
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \bicaption{\quad 应用与干扰混部场景下不同内核配置对BE应用平均运行时间的影响}{\quad The impact of different kernel configurations on the average runtime of BE applications in a co-location scenario of applications and interference}
...
\bicaption{\quad 应用与干扰混部场景下CPU感知调度策略对LC应用QoS保障效果}{\quad The effect of CPU-aware scheduling strategies on QoS assurance for LC applications in a co-location scenario of applications and interference}
...
\begin{figure}[H]
    ...
    \end{subfigure}
\bicaption{\quad 应用与干扰混部场景下的虚拟机总CPU利用率}{\quad 
Total CPU utilization of virtual machines in a mixed deployment scenario of applications and interference}
\label{fig:cpu_usage}
\end{figure}
...
\begin{figure}[H]
    ...
    \end{subfigure}
\bicaption{\quad SMT混部场景下CPU感知调度策略对LC应用QoS保障效果}{\quad The effect of CPU-aware scheduling strategy on QoS assurance for LC applications in an SMT co-location scenario}
...
\begin{figure}[H]
    ...
    \end{subfigure}
\bicaption{\quad SMT混部场景下不同调度策略对Redis延迟稳定性的影响}{\quad The impact of different scheduling strategies on Redis latency stability in an SMT co-location scenario}
...
\begin{figure}[!htbp]
    ...
    \end{subfigure}
\bicaption{\quad LC应用负载与Epoll Wait事件数量的关系}{\quad The relationship between LC application load and the number of Epoll Wait events}
...
\begin{figure}[H]
    ...
    \bicaption{\quad 应用与干扰混部一段时间内的CPU资源使用情况}{\quad The CPU resource utilization of applications and interference over a period of time}
