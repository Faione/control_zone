...
\begin{itemize}
    \item Host层级指标如表~\ref{tab:host_metric}所示, 指标来源分别为Kernel、
...
\end{table}
    ...
    \item App层级的指标主要从日志、内嵌的指标采集接口以及Envoy网络代理上采集。以Redis为例，通过Envoy网络代理接管传入Redis的所有流量，并在代理中记录各种操作的数量、延时统计等信息。
