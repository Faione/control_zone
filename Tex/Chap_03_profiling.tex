...
\begin{figure}[!htbp]
    ...
    \bicaption{\quad vHost Net模拟网络设备后端处理流程}{\quad 
    The backend processing flow of the vHost Net emulated network device}
    \label{fig:vhost_net}
\end{figure}
...
\begin{figure}[!htbp]
    ...
    \bicaption{\quad Black Box观测系统架构}{\quad Black Box observability system architecture}
...
\begin{figure}[!htbp]
    ...
    \bicaption{\quad Black Box观测系统指标采集与Exporters设计}{\quad Metric collection and exporters design in Black Box observation system}
    \label{fig:exporters} 
\end{figure}
...
\begin{enumerate}
    ...
    \begin{figure}[H]
        ...
        \bicaption{\quad Syscall中eBPF程序的执行过程}{\quad The execution process of eBPF programs in syscalls}
...
\begin{table}[!htbp]
    \bicaption{\quad Host层级指标}{\quad Host-level metrics}% caption
...
\begin{table}[!htbp]
    \bicaption{\quad Hypervisor层级指标}{\quad Hypervisor-level metrics}% caption
...
\begin{table}[!htbp]
    \bicaption{\quad C6型号服务器信息}{\quad Information about C6 model server}% caption
    \label{tab:c6_info}
    \footnotesize% fontsize
    \setlength{\tabcolsep}{4pt}% column separation
    \renewcommand{\arraystretch}{1.25}% row space 
...
\begin{figure}[H]
    ...
    \begin{subfigure}[b]{0.49\textwidth}
        ...
        \label{fig:cof_mem}
    \end{subfigure}
\bicaption{\quad 基准实验中不同应用资源均值的变异系数比较}{\quad Comparison of the coefficient of variation of the average resource usage for different applications in benchmark experiments}
...
具体到每个应用的指标差异如图~\ref{fig:resource_affinity_0}所示，其中横坐标是指标名称，纵坐标是分别是各项指标极差、均值与标准差通过变异系数归一化后的结果，并使用不同颜色的柱状图来区分。不同应用对于主要的5种系统资源的需求呈现处较大的差异：
...
\begin{figure}[H]
    ...
    \end{subfigure}
\bicaption{\quad Redis与Memcached资源指标统计值的变异系数对比}{\quad Comparison of the coefficient of variation of resource metric statistics between Redis and Memcached}
...
\begin{figure}[H]
    ...
    \end{subfigure}
\bicaption{\quad Nginx与Kafka资源指标统计值的变异系数对比}{\quad Comparison of the coefficient of variation of resource metric statistics between Nginx and Kafka}
...
\begin{figure}[H]
    ...
    \end{subfigure}
\bicaption{\quad Elasticsearch与Render资源指标统计值的变异系数对比}{\quad Comparison of the coefficient of variation of resource metric statistics between Elasticsearch and Render}
...
\begin{table}[H]
    \centering
    \bicaption{\quad stress\_ng干扰参数列表}{\quad stress\_ng interference parameter list}% caption
    \footnotesize% fontsize
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \bicaption{\quad stress\_ng不同配置下资源干扰效果}{\quad Resource interference effects of stress\_ng under different configurations}
...
\bicaption{\quad 不同资源干扰下Redis的服务性能指标劣化情况}{\quad Degradation of Redis QoS metric under different resource interference}
